...
describe('Diagnostics reports lantern recipients matching notify.ts (BUG-B)', () => {
  ...
  it("uses the same eligibility filters as notify.ts:notifyBellRing", () => {
    // notifyBellRing filters by: role='caregiver', villageGroup='covey',
    // notifyBellRinging=true. The diagnostic must mirror this exactly so
    // its verdict matches what a real push attempt would see.
    expect(/eq\(users\.role,\s*'caregiver'\)/.test(diagSrc)).toBe(true);
    expect(/eq\(users\.villageGroup,\s*'covey'\)/.test(diagSrc)).toBe(true);
    expect(/eq\(users\.notifyBellRinging,\s*true\)/.test(diagSrc)).toBe(true);

    // Cross-check: notifyBellRing must also still use these — if it drifts,
    // the diagnostic's verdict becomes a lie.
    expect(/eq\(users\.role,\s*'caregiver'\)/.test(notifySrc)).toBe(true);
    expect(/eq\(users\.villageGroup,\s*'covey'\)/.test(notifySrc)).toBe(true);
