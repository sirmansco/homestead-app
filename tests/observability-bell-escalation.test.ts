...
vi.mock('@/lib/notify', () => ({
  notifyBellEscalated: vi.fn().mockResolvedValue(undefined),
}));
...
describe('T-A — bell_escalated structured log on successful escalation', () => {
  beforeEach(() => { vi.clearAllMocks(); });

  it('successful escalation emits bell_escalated log', async () => {
    vi.mocked(db.select).mockReturnValue(
      makeSelectChain([{ escalatedAt: null }]) as unknown as ReturnType<typeof db.select>,
    );
    vi.mocked(db.update).mockReturnValue(
      makeUpdateChain([{ id: 'bell-1' }]) as unknown as ReturnType<typeof db.update>,
    );

    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    await escalateBell('bell-1');

    const calls = logSpy.mock.calls.map(c => c[0] as string);
    const escalatedLog = calls.find(c => {
      try { return JSON.parse(c).event === 'bell_escalated'; } catch { return false; }
    });
    expect(escalatedLog).toBeDefined();
    const parsed = JSON.parse(escalatedLog!);
    expect(parsed.bellId).toBe('bell-1');
    expect(parsed.at).toBeDefined();

    logSpy.mockRestore();
  });

  it('race-lost escalation (UPDATE returns 0 rows) emits no bell_escalated log', async () => {
    vi.mocked(db.select).mockReturnValue(
      makeSelectChain([{ escalatedAt: null }]) as unknown as ReturnType<typeof db.select>,
    );
    vi.mocked(db.update).mockReturnValue(
  ...
  it('already-escalated bell (escalatedAt not null) emits no log', async () => {
    vi.mocked(db.select).mockReturnValue(
